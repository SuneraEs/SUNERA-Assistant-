...
async def start_solar(update: Update, context: ContextTypes.DEFAULT_TYPE):
...
async def solar_parse(update: Update, context: ContextTypes.DEFAULT_TYPE):
    lang = context.user_data.get("lang", pick_lang(update.effective_user.language_code))
    parts = (update.message.text or "").replace(",", ".").split()
    if len(parts) not in (2, 3):
        await update.message.reply_text(t("solar_badfmt", lang)); return ASK
    try:
        consumption = float(parts[0])  # kWh/month
        tariff = float(parts[1])       # €/kWh
        ...
        msg = t("solar_result", lang).format(
            kw=round(kw_needed, 2),
            cost=round(cost, 0),
            cperkW=int(cost_per_kw),
            gen=int(yearly_gen),
            save=int(yearly_save),
...
async def solar_cancel(update: Update, context: ContextTypes.DEFAULT_TYPE):
