...
ADMIN_CHAT_ID = int(os.getenv("ADMIN_CHAT_ID", "0"))  # 0 => уведомления админу отключены
...
def get_gsheets_credentials_dict():
    """Читает ключ сервисного аккаунта из ENV GSHEETS_JSON или файла gsheets.json."""
    import json, io
...
# ========= RUNTIME TWEAKS =========
ANTI_FLOOD_WINDOW_SEC = int(os.getenv("ANTI_FLOOD_WINDOW_SEC", "1"))   # 1 сек между сообщениями
SOLAR_COST_PER_KW = float(os.getenv("SOLAR_COST_PER_KW", "1000"))     # €/kW по умолчанию
SOLAR_PERFORMANCE = float(os.getenv("SOLAR_PERFORMANCE", "0.75"))     # системный КПД
